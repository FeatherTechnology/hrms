...
if($notice_per_served==1){

$query = $pdo->query("UPDATE `users` SET `relieve_date`= '$last_wrk_day',`update_login_id`='$user_id',`updated_on`=now() WHERE staff_name_id = '$staff_profile_id' ");

}else{
$query = $pdo->query("UPDATE `users` SET `status`='1', `relieve_date`= now(),`update_login_id`='$user_id',`updated_on`=now() WHERE staff_name_id = '$staff_profile_id' ");
