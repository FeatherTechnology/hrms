  ...
  $.each(response, function (index, item) {
    ...
    $.each(columnMapping, function (key, value) {
      ...
      if (value === "sno") {
        $("<td>")
          .text(index + 1)
          .appendTo(row); // Add serial number
...
function setDropdownScripts() {
  ...
  $(document).click(function (event) {
    ...
    if (
      ...
      !target.closest(".logout-link").length
    ) {
        ...
        img.onload = () => {
          ...
          if (width > maxImageDimension || height > maxImageDimension) {
            const scale = Math.min(
              maxImageDimension / width,
              maxImageDimension / height,
            );
            width *= scale;
            height *= scale;
          }
...
function checkBankTransactionDetails(crdrType, bankId, transId, amount) {
  return $.post(
    "include/common/getBankTransactionDetails.php",
    {
      crdrType,
      bankId,
      transId,
